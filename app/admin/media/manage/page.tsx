...
const ManageMedia = () => {
    ...
    const fetchMediaItems = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API}/api/audios`
        );
        const data = await response.json();
        setMediaItems(data.audioFiles); // Dữ liệu audioFiles từ API
      } catch (error) {
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      {/* Hiển thị danh sách media dưới dạng danh sách */}
      <div className="space-y-6">
        {filteredItems.map((item) => (
          <div key={item.id} className="border p-4 rounded-md shadow-sm">
            ...
            <p>Uploaded by: {item.userId}</p>
            <p>Size: {item.size ? item.size : "Unknown"}</p>
            <p>Topic: {item.topic || "Chưa có chủ đề"}</p>
            <p>Upload Date: {new Date(item.uploadDate).toLocaleString()}</p>
